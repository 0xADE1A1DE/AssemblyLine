...
static void
overide_opd_size(struct instr* all_instr, unsigned int* rm)
{
...
static unsigned int
get_vector_rex_prefix(struct instr* all_instr, asm_reg m, asm_reg r)
{
...
unsigned int
get_rex_prefix(struct instr* all_instr, struct operand* m, struct operand* r)
{
...
uint8_t
get_reg(struct instr* instrc, struct operand* m, int r)
{
    ...
    if ( m->index == reg_none )
    {
        instrc->hex.reg = instrc->mod_disp | ((r & VALUE_MASK) << 3) | (m->reg & VALUE_MASK);
        return EXIT_SUCCESS;
    }
    else if ( (m->index & REG_MASK) == spl )
    {
        FAIL_IF_MSG((m->reg & REG_MASK) == spl || instrc->sib_disp, "error stack pointer register is not scalable\n");
    }
    // SIB addressing mode
    instrc->is_sib  = true;
    instrc->hex.sib = instrc->sib_disp | ((m->index & VALUE_MASK) << 3) | (m->reg & VALUE_MASK);
