 ...
 */
static int
check_registers(struct instr* check_instr)
{
 ...
 */
static int
line_to_instr(struct instr* instr_data, char* filtered_asm_str)
{
    ...
    FAIL_IF_VAR(instr_data->key == INSTR_ERROR, "unsupported or illegal instruction: %s\n", instr_data->instruction);
    if ( instr_data->imm && INSTR_TABLE[instr_data->key].type == CONTROL_FLOW )
    {
        if ( IN_RANGE(instr_data->cons, 0xffffff80, 0xffffffff) )
            ...
            FAIL_IF_MSG(instr_data->cons > 0x7f && instr_data->keyword.is_short, "cannot set a long jump to short\n");
    }
    ...
    FAIL_IF_VAR(check_registers(instr_data), "Invalid register for instruction: %s\n", instr_data->instruction);
 ...
 */
static int
filter_assembly_str_fsa(const char unfiltered_str[], char filter_str[])
{

    filter_op filter_state = BEGIN;
    int j                  = 0;
    int i                  = 0;
    while ( unfiltered_str[i] != ';' && unfiltered_str[i] != '%' && unfiltered_str[i] != '\r' &&
 ...
 */
static int
str_to_instr(struct instr* instr_data, const char unfiltered_str[], int* read_len)
{
    ...
    // skip comments/macro
    while ( unfiltered_str[ch_pos] != '\n' && unfiltered_str[ch_pos] != '\r' && unfiltered_str[ch_pos] != '\0' )
        ch_pos++;
    if ( unfiltered_str[ch_pos] == '\n' || unfiltered_str[ch_pos] == '\r' )
        ch_pos++;
    *read_len = ch_pos;
    // map filter_str to instr_data if not it is not a label or header
    if ( filter_str[0] != '\0' && strstr(filter_str, "section") == NULL && strstr(filter_str, "global") == NULL &&
         strchr(filter_str, ':') == NULL )
 ...
 */
static void
debug_without_chunksize(int opcode_len, uint8_t* ptr)
{
 ...
 */
static void
debug_with_chunksize(uint8_t* buf, int opcode_pos, size_t chunk_size)
{
 ...
 */
static int
check_len_or_resize(assemblyline_t al, int buf_pos)
{

    if ( buf_pos + BUFFER_TOLERANCE > al->buffer_len )
    {
        FAIL_IF_VAR(al->external, "exceeded memory buffer length: al->buffer_len = %d\n", al->buffer_len)
#if defined(__linux__)
        // resize internal memory buffer
        void* resize = mremap(al->buffer, al->buffer_len, al->buffer_len + MEM_BUFFER, MREMAP_MAYMOVE);
 ...
 */
static int
assemble_counting_chunks(assemblyline_t al, struct instr* new_instr, int* buf_pos, int* chunk_brks)
{
 ...
 */
static int
assemble(assemblyline_t al, struct instr* new_instr, int* buf_pos)
{
 ...
 */
static int
assemble_with_chunk_fitting(assemblyline_t al, struct instr* new_instr, int* buf_pos)
{
    ...
    do
    {
        ...
        if ( written_length <= free_chunk_space || written_length >= al->chunk_size ||
