...
// expected results
uint64_t expected_out[] = {0x165651D83282, 0x18883C3EA80B, 0x1EDBDFE96957, 0x238587B25BAC3, 0x182355AC294};

int
execute_test(void (*exe)(uint64_t*, uint64_t*, ...))
{
    ...
    for ( int it = 0; it < 5; it++ )
    {
        ...
        {
            fprintf(stderr, "👎 out[it]=%llu (expected_out[it]=%llu)\n", out[it], expected_out[it]);
            return EXIT_FAILURE;
        }
    }
    fprintf(stderr, "👍\n");
...
main()
{
    ...
    {
        fprintf(stderr, "cur_B.asm with chunk fitting did not produce expected results\n");
        return 1;
    }
    asm_set_offset(al, 0);
    ...
    uint8_t* buffer =
#if defined(__linux__)
        mmap(NULL, sizeof(uint8_t) * 60000, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    if ( buffer == MAP_FAILED )
    {
#else
        (uint8_t*)VirtualAlloc(NULL, sizeof(uint8_t) * 60000, MEM_PRIVATE | MEM_COMMIT, PAGE_EXECUTE_READWRITE);
    if ( buffer == NULL )
    {
#endif
