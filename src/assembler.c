...
static const uint8_t* FIXED_NOP_LENGTH[] = {
    ...
    (uint8_t[]) {0x66, 0x66, 0x66, 0x0F, 0x1F, 0x84, 0x00, 0x00, 0x00, 0x00, 0x00}};
 ...
 */
static int
assemble_const(unsigned long constant, unsigned char ptr[])
{
 ...
 */
static bool
check_zero(struct instr* instruc, unsigned long saved_imm, instr_type type)
{
    // check for signed 32bit overflow
    if ( IN_RANGE(saved_imm, NEG32BIT_CHECK, MAX_UNSIGNED_32BIT) && !instruc->reduced_imm && type != CONTROL_FLOW )
    {
        // nasm immediate register handling disabled
        if ( !(instruc->assembly_opt & ASM_OPT_NASM) )
            return true;
        // nasm immediate register handling disabled
 ...
 */
static int
assemble_imm(struct instr* instruc, unsigned char ptr[])
{
    int ptr_pos               = 0;
    ...
    bool zero_pad = ((type != CONTROL_FLOW &&       // it must not be CONTROL_FLOW
 ...
 */
static int
assemble_mem(struct instr* instruc, unsigned char ptr[])
{
...
static int
assemble_VEX(struct instr* instruc, unsigned char ptr[], unsigned int vex)
{
    int i                  = 0;
 ...
 */
static int
assemble_instr(struct instr* instruc, unsigned char ptr[])
{
    ...
    if ( (instruc->opd[0].reg & BIT_MASK) == BIT_16 || instruc->hex.is_66H || instruc->keyword.is_word )
        ptr[ptr_pos++] = 0x66;
 ...
 */
int
assemble_asm(struct instr* instruc, uint8_t* dest)
{
